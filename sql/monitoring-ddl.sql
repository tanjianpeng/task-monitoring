...
CREATE TABLE monitor_task (
    ...
    plan_start_time DATETIME NOT NULL COMMENT '跑批预计开始时间',
    plan_end_time DATETIME NOT NULL COMMENT '跑批预计结束时间',
    default_cost_minutes INT DEFAULT 0 COMMENT '预计耗时(单位分钟)',
    avg_cost_minutes INT DEFAULT 0 COMMENT '历史平均耗时(单位分钟)',
    delay_minutes INT NOT NULL COMMENT '延迟分钟数',
    frequency VARCHAR(1) COMMENT ' 频率 D:日 M:月 Y:年',
    display_flag VARCHAR(1) NOT NULL DEFAULT '1' COMMENT '是否大屏展示，1是，0否',
    pos_x INT NOT NULL DEFAULT 0 COMMENT '横坐标X',
    pos_y INT NOT NULL DEFAULT 0 COMMENT '纵坐标Y',
    status VARCHAR(1) NOT NULL DEFAULT '1' COMMENT '状态，1启用，0停用',
...
CREATE TABLE monitor_task_instance (
    ...
    predict_end_time DATETIME COMMENT '预计结束时间',
    delayed_flag INTEGER(1) NOT NULL DEFAULT '0' COMMENT '是否延迟，1是，0否',
    timeout_flag INTEGER(1) NOT NULL DEFAULT '0' COMMENT '是否超时，1是，0否',
    result_status INTEGER(1) VARCHAR(32) COMMENT '执行结果状态',
    is_flag INTEGER(1) NOT NULL DEFAULT '0' COMMENT '是否展示，1否，0是',
...
CREATE TABLE monitor_task_log (
    ...
    status VARCHAR(32) NOT NULL COMMENT '动作状态 start/stop/restart',
    system_code VARCHAR(64) NOT NULL COMMENT '所属系统编码',
